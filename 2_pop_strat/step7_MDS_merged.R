data<- read.table(file="./outputs/MDS_merge2.mds", header=TRUE)
race<- read.table(file="./outputs/ethn_file_concatenated.txt", header=TRUE)
datafile<- merge(data, race, by=c("IID","FID"))
head(datafile)

pdf("./outputs/MDS.pdf",width=7,height=7)
